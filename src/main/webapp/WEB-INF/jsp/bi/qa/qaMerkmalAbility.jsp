...
<body>
	...
	<div id="dztDIV" class="easyui-dialog" data-options="title: '测量数据',width: $(window).width(),height: $(window).height(),closed: true,cache: false,modal: true">
 	...
 	<div id="tableBar" style="padding: 5px">
 		开始时间：<input id="qaMerkmalStartTime" type="text" class="easyui-datetimebox" data-options="editable:false,onChange:function(newDate,oldDate){qaDateTimeBoxSearch(newDate,1)},onShowPanel:function(){  
	        																							$(this).datetimebox('spinner').timespinner('setValue','07:00:00');},buttons:startbuttons ">
 		结束时间：<input id="qaMerkmalEndTime" type="text" class="easyui-datetimebox" data-options="editable:false,onChange:function(newDate,oldDate){qaDateTimeBoxSearch(newDate,2)},onShowPanel:function(){  
	        																							$(this).datetimebox('spinner').timespinner('setValue','07:00:00');},buttons:endbuttons">
 	...
 	<div id="qdasImgBoxBg3" class="easyui-window" title="Q-DAS" style="width:550px;height:750px" data-options="collapsible:false,modal:true,minimizable:false,maximizable:false,closed:true"></div>
	...
	var dataFlag=0;
					...
					if(data.mess==-1){
						...
						$('#qaMerkmalDetailsTable').datagrid('updateRow',{
							...
							row: {
								...
								'<img title=\"直方图\" onclick="showImgDivBg3(\''+data.bg3+'\');" style="width:32px;height:30px;cursor:pointer;" src=\"data:image/bmp;base64,'+data.bg3+'\"/>'
							}
						});
					}
		...
		function showImgDivBg3(imgstr){
			$('#qdasImgBoxBg3').empty();
  			$('#qdasImgBoxBg3').append("<img  style=\"width: 510px;height: 730px\" src=\"data:image/bmp;base64,"+imgstr+"\">")
  			$('#qdasImgBoxBg3').window('open');
		}
		...
		var startbuttons = $.extend([], $.fn.datetimebox.defaults.buttons);
        startbuttons.splice(2, 1, {
            text: '清空',
            handler: function(target){
                $('#qaMerkmalStartTime').datetimebox('setValue','');
            }
        });
        var endbuttons = $.extend([], $.fn.datetimebox.defaults.buttons);
        endbuttons.splice(2, 1, {
            text: '清空',
            handler: function(target){
                $('#qaMerkmalEndTime').datetimebox('setValue','');
            }
        });
	</script>
</body>
