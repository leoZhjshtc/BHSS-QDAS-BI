  ...
  <body>
	  <div class="easyui-layout" fit="true">
	  	<div data-options="region:'center'">
	  		<div class="easyui-layout" fit="true">
	  			<div data-options="region:'west',split:false,border:false,collapsible:false" style="width:40%;">
	  				<img alt="" src="<%=basePath%>resources/images/factoryImg/${factoryBgImg }" style="width:100%;height:100%">
	  			</div>
	  			<div id="qaPlChartDiv" data-options="region:'center',onResize:function(){qaProductLineChartResize()}"></div>
	  		</div> 
	  	</div>
	    <div data-options="region:'south',split:false,border:false,collapsible:false" style="height:70%;">
	   ...
	   <!--  <div id="qaProductLineChart" data-options="region:'center',border:false,onResize:function(){qaProductLineChartResize()}" style="">
	    </div> -->
	  </div>
  		...
  		$(function(){
    		...
    		function plCPKFormatter(value,row,index){
    			...
    			for(var i=0;i<row.COUNTLIST.length;i++){
    				...
    				for(var j=0;j<row.COUNTLIST[i].WVCOUNTLIST.length;j++){
    					if(row.COUNTLIST[i].WVCOUNTLIST[j].MEMERKART != '1'&row.COUNTLIST[i].WVCOUNTLIST[j].CPKALARM == '1'){
    						warnCPKCount +=1;
    					}else if(row.COUNTLIST[i].WVCOUNTLIST[j].MEMERKART != '1'&row.COUNTLIST[i].WVCOUNTLIST[j].CPKALARM == '2'){
    						failCPKCount +=1;
    					}
    				}
    			}
