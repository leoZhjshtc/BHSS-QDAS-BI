			...
			<div data-options="region:'center'" style="background-image: url(<%=basePath%>resources/images/drawing/${workShopDrawing });background-repeat: no-repeat;background-size:100% 100%;">
					<c:forEach items="${plList}" var="map">
						<a id="${map.productLine_name }" onclick="editPl(this)" class="easyui-linkbutton c1" data-options="size:'large'" 
							style="width:150px;height: 120px;position: absolute;top: ${map.productLine_topPosition};left: ${map.productLine_leftPosition}" >${map.productLine_name }</a>
					...
					<div id="newPlDopdiv" class="easyui-draggable" data-options="onDrag:onDrag"><a class="easyui-linkbutton c6" data-options="size:'large'" style="width:150px;height: 120px;" >新产线</a></div>
				...
				$.ajax({
					...
					success:function(data){
						if(data.message==1){
							...
							$('#plCenterbox').panel('refresh','<%=basePath%>system/initProductLineSetup?factoryName='+encodeURI($('#factoryNamePL').val())+'&workShopDrawingStr='+encodeURI($('#workShopDrawingPL').val())+'&workShopName='+encodeURI($('#workShopNamePL').val()));
						}
					}
				})
			}
